...
def editEmailConfig():
    try:
        ...
        print(f"Error in editEmailConfig: {str(e)}")
...
def editConfig():
    try:
        ...
        if not darkMode or alerts is None:  # Changed to check for None instead of falsy
            return jsonify({"error": "Invalid input"}), 400
        
        claims = get_jwt()
        userID = claims.get('user_id')
        
        listOfAlerts = list(alerts) if alerts else []
        ...
        print(f"Error in editConfig: {str(e)}")
        return jsonify({"error": "Internal server error"}), 500
