...
def get_sims_paginated():
    try:
        ...
        # Get total count and paginated SIMs
        total = collection.count_documents({})
        sims = list(collection.find({}).skip(skip).limit(per_page))
        
        # Get vehicle data for IMEI mapping
        vehicle_collection = db['vehicle_inventory']
        ...
        for sim in sims:
            ...
            if sim.get('DateIn'):
                try:
                    if hasattr(sim['DateIn'], 'strftime'):
                        sim['DateIn'] = sim['DateIn'].strftime('%d-%m-%Y')
                    else:
                        date_obj = datetime.strptime(str(sim['DateIn']), '%Y-%m-%d')
                        sim['DateIn'] = date_obj.strftime('%d-%m-%Y')
                except:
                    sim['DateIn'] = str(sim['DateIn'])
            
            if sim.get('DateOut'):
                try:
                    if hasattr(sim['DateOut'], 'strftime'):
                        sim['DateOut'] = sim['DateOut'].strftime('%d-%m-%Y')
                    else:
                        date_obj = datetime.strptime(str(sim['DateOut']), '%Y-%m-%d')
                        sim['DateOut'] = date_obj.strftime('%d-%m-%Y')
                except:
                    sim['DateOut'] = str(sim['DateOut'])
            
            # Format last edited date
            last_edited_at = sim.get('lastEditedAt', '')
            if last_edited_at:
                if hasattr(last_edited_at, 'strftime'):
                    sim['lastEditedAt'] = last_edited_at.strftime('%d-%m-%Y %I:%M:%S %p')
                else:
                    try:
                        dt = datetime.fromisoformat(str(last_edited_at).replace('Z', '+00:00'))
                        sim['lastEditedAt'] = dt.strftime('%d-%m-%Y %I:%M:%S %p')
                    except:
                        pass
        ...
        print(f"Error in get_sims_paginated: {str(e)}")
    ...
    try:
        ...
        if status != 'All':
            if status in ['Active', 'Inactive']:
                ...
                # For status-based filtering, we'll handle the logic after fetching
                pass
        
        # Get total count for pagination
        total = collection.count_documents(query)
        all_sims = list(collection.find(query).skip(skip).limit(per_page))
        ...
        for sim in all_sims:
            ...
            sim_data = {
                ...
                'lastEditedAt': sim.get('lastEditedAt', '')
            }
            
            # Format dates
            if sim_data['DateIn']:
                try:
                    if hasattr(sim_data['DateIn'], 'strftime'):
                        sim_data['DateIn'] = sim_data['DateIn'].strftime('%d-%m-%Y')
                    else:
                        date_obj = datetime.strptime(str(sim_data['DateIn']), '%Y-%m-%d')
                        sim_data['DateIn'] = date_obj.strftime('%d-%m-%Y')
                except:
                    sim_data['DateIn'] = str(sim_data['DateIn'])
            
            if sim_data['DateOut']:
                try:
                    if hasattr(sim_data['DateOut'], 'strftime'):
                        sim_data['DateOut'] = sim_data['DateOut'].strftime('%d-%m-%Y')
                    else:
                        date_obj = datetime.strptime(str(sim_data['DateOut']), '%Y-%m-%d')
                        sim_data['DateOut'] = date_obj.strftime('%d-%m-%Y')
                except:
                    sim_data['DateOut'] = str(sim_data['DateOut'])
        ...
        return jsonify({
            "total": len(results),  # Return filtered count
            "page": page,
            "per_page": per_page,
            "sims": results
        })
...
def search_sims():
    try:
        ...
        query = {
            "$or": [
                {"MobileNumber": {"$regex": f"{search_query}$", "$options": "i"}}, 
                {"SimNumber": {"$regex": f"{search_query}$", "$options": "i"}},   
        ...
        # Get total count for pagination
        total = collection.count_documents(query)
        matching_sims = list(collection.find(query).skip(skip).limit(per_page))
        ...
        print(f"Error in search_sims: {str(e)}")
        return jsonify({'error': 'Failed to search SIMs'}), 500
