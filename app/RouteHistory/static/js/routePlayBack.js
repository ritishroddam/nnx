...
let map, pathPolyline, carMarker;
let pathCoordinates = [];
const carContent = createCarElement();
...
window.gm_authFailure = () =>
  alert("Google Maps failed to load. Check API key!");
...
async function initMap(darkMode = true) {
  try {
    ...
    map = new Map(document.getElementById("map"), {
      zoom: 10,
      center: { lat: 0, lng: 0 },
      mapId: darkMode ? "44775ccfe2c0bd88" : "8faa2d4ac644c8a2",
      gestureHandling: "greedy",
...
// Core Functionality
// ========================
function plotPathOnMap(coordinates) {
  cleanupPreviousPath();
  pathCoordinates = coordinates;

  if (coordinates.length > 0) {
    updateMapViewport(coordinates);
    createPathPolyline(coordinates);
    createStartEndMarkers(coordinates);
    createDirectionArrows(coordinates);
    resetCarMarker(coordinates[0]);
  }
}
...
function moveCar() {
  if (
    ...
    animationState.currentPathIndex >= pathCoordinates.length - 1
  ) {
    return resetAnimation();
  }

  const start = pathCoordinates[animationState.currentPathIndex];
  const end = pathCoordinates[animationState.currentPathIndex + 1];
  ...
  const steps = Math.max(1, Math.min(1000, Math.floor(distance / 10)));
  const bearing = calculateBearing(start, end);
  carContent.style.transform = `rotate(${bearing}deg)`;

  animateMovement(start, end, steps);
}
...
function animateMovement(start, end, steps) {
  ...
  const stepDuration = Math.max(10, 50 / animationState.currentSpeed);
  ...
  const animate = () => {
    ...
    const newPosition = {
      lat: start.lat + latStep * stepIndex,
      lng: start.lng + lngStep * stepIndex,
    };

    updateCarPosition(newPosition);
    handleMapViewport(newPosition);
...
function createCarElement() {
  ...
  elem.src = "/static/images/car_green.png";
...
function updateProgress() {
  const progress =
    (animationState.currentPathIndex / (pathCoordinates.length - 1)) * 100;
...
function handleMapViewport(position) {
  const bounds = map.getBounds();
  if (!bounds || !bounds.contains(position)) {
    const newCenter = google.maps.geometry.spherical.interpolate(
      map.getCenter(),
      position,
      0.1
    );
...
function resetAnimation() {
  ...
  if (pathCoordinates.length) {
    carMarker.position = pathCoordinates[0];
  }
}

function setSpeed(multiplier) {
  animationState.currentSpeed = multiplier;
  if (animationState.isPlaying) {
    stopCarAnimation();
    startCarAnimation();
  }
...
// Memory Management
// ========================
function cleanupPreviousPath() {
  if (pathPolyline) pathPolyline.setMap(null);
  animationState.markersPool.forEach((m) => (m.map = null));
  animationState.markersPool = [];
}

function createStartEndMarkers(coordinates) {
  const startMarker = getPooledMarker();
  startMarker.position = coordinates[0];
  startMarker.content = createMarkerContent("Start", "green");
  startMarker.map = map;

  const endMarker = getPooledMarker();
  endMarker.position = coordinates[coordinates.length - 1];
  endMarker.content = createMarkerContent("End", "red");
  endMarker.map = map;
}

function getPooledMarker() {
  return (
    animationState.markersPool.pop() ||
    new google.maps.marker.AdvancedMarkerElement({ map })
  );
}
