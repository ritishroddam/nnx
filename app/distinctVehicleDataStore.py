...
def update_distinct_atlanta(socketio):
    try:
        ...
        existing_documents = {doc['imei']: doc for doc in distinct_atlanta_collection.find()}
        ...
        for doc in all_documents:
            ...
            # Only keep the most recent data for each IMEI
        ...
        # Clear the distinct collection and insert the new documents
        ...
        # Compare the updated documents with existing ones, and emit changes
        count = 0
        for imei, doc in distinct_documents.items():
            if imei in existing_documents:
                # Compare with existing data
                if doc != existing_documents[imei]:
                    doc.pop('_id', None)  # Remove _id for the emission
...
def run_distinct_vehicle_data_store(socketio):
    while True:
        ...
        eventlet.sleep(120)  # Sleep for 120 seconds before the next round
