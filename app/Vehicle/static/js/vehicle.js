...
function updateVehicleCard(data) {
  ...
  const cardContent = `
    ...
    <div class="vehicle-info">
      ...
      <strong>GSM Signal:</strong> ${gsmIcon} ${
    data.gsm && !isNaN(data.gsm) ? `(${data.gsm})` : ""
  } <br>
      ...
      <strong>Data:</strong> <a href="/routeHistory/vehicle/${
  ...
  try {
    ...
    const enhancedVehicles = await Promise.all(
      vehicles.map(async (vehicle) => {
        ...
        const distanceEntry = distances.find(
          (d) => d.registration === vehicle.LicensePlateNumber
        );
        const distanceToday = distanceEntry
          ? `${distanceEntry.distance.toFixed(2)} km`
          : "N/A";
        ...
        return {
          ...
          address:
            vehicle.address ||
            (await fetchLocation(vehicle.latitude, vehicle.longitude).catch(
              () => "No address found"
            )),
        };
      })
...
function createInfoWindowContent(
  ...
  return `<div class="info-window show">
            <strong><span style="color: #336699;">${
              device.LicensePlateNumber || "Unknown"
            }:</span></strong> <br>
            <hr>
            ...
            <p><strong>Distance Today:</strong> ${
              device.distance_today || "N/A"
            } km</p>
            <p><strong>Ignition:</strong> ${ignitionIcon}</p>
            <p><strong>GSM Signal:</strong> ${gsmIcon}</p>
            <p><strong>SOS Status:</strong> ${
              device.sos === "1"
                ? '<span class="text-danger">ACTIVE</span>'
                : "Inactive"
            }</p>
