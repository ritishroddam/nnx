    ...
    div.innerHTML=`
      <div class="report-info report-open" data-id="${r._id}">
        ...
        <div class="report-meta">
          <span><b>Type:</b> ${r.report_type.replace(/-/g,' ').replace(/\b\w/g, l => l.toUpperCase())}</span>
          <span><b>Generated By:</b> ${r.user_id}</span>
          <span><b>Vehicle:</b> ${r.vehicle_number}</span>
          <span><b>Size:</b> ${(r.size/1024).toFixed(1)} KB</span>
          <span><b>Time:</b> ${new Date(r.generated_at).toLocaleString()}</span>
        </div>
      </div>
