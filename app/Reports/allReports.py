...
#                 'course', 'checksum', 'reserve1', 'reserve2',
...
#     # Get current date in string format (ddmmyy)
#     now = datetime.now()
#     current_date_str = now.strftime("%d%m%y")
#     yesterday_date_str = (now - timedelta(days=1)).strftime("%d%m%y")
...
#         # Add date range filter for time-series data (string comparison)
#         if collection == 'atlanta' and date_range:
#             if date_range == "last24hours":
#                 # For last 24 hours, we need to handle crossing midnight
#                 query['$or'] = [
#                     {
#                         'date': current_date_str,
#                         'time': {'$gte': (now - timedelta(hours=24)).strftime("%H%M%S")}
#                     },
#                     {
#                         'date': {'$gt': yesterday_date_str}
#                     }
...
#             elif date_range == "yesterday":
#                 query['date'] = yesterday_date_str
#             elif date_range == "last7days":
#                 # Get dates for last 7 days as strings
#                 date_strings = [(now - timedelta(days=i)).strftime("%d%m%y") for i in range(7)]
#                 query['date'] = {'$in': date_strings}
#             elif date_range == "last30days":
#                 # Get dates for last 30 days as strings
#                 date_strings = [(now - timedelta(days=i)).strftime("%d%m%y") for i in range(30)]
#                 query['date'] = {'$in': date_strings}
