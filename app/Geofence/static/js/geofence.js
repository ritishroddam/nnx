...
async function geofenceMapFunction() {
  ...
  const el = document.getElementById('geofenceMap');
  if (!el) {
    console.error('Map element #geofenceMap not found');
    return;
  }

  geofenceMap = new Map(el, {
    center: { lat: 13.013904, lng: 77.648613 },
    zoom: 5,
    disableDoubleClickZoom: true,
  });

  draw = new TerraDraw({
    adapter: new TerraDrawGoogleMapsAdapter({ map: geofenceMap, lib: google.maps, coordinatePrecision: 9 }),
    modes: [
      ...
      new TerraDrawRectangleMode({ editable: true }),
      new TerraDrawCircleMode({ editable: true }),
    ],
  });
...
function wireUiButtons() {
  ...
  if (polylineBtn) polylineBtn.addEventListener('click', () => setMode('linestring', polylineBtn));
...
function featureToCoordinatesPayload(feature) {
  const mode = (feature.properties && feature.properties.mode) || feature.properties?.mode || '';

  if (mode === 'rectangle' || (feature.geometry && feature.geometry.type === 'Polygon' && mode === 'rectangle')) {
  ...
  if (mode === 'linestring' || feature.geometry?.type === 'LineString') {
    const coords = feature.geometry.coordinates || [];
    const points = coords.map(([lng, lat]) => ({ lat, lng }));
    return { shape_type: 'polyline', coordinates: { points } };
  }

  if (mode === 'circle' || feature.properties?.radius || feature.properties?._radius) {
    const center = feature.properties?.center || (feature.geometry?.type === 'Point' ? { lat: feature.geometry.coordinates[1], lng: feature.geometry.coordinates[0] } : null);
    const radius = feature.properties?.radius ?? feature.properties?._radius;
    if (center && typeof radius === 'number') return { shape_type: 'circle', coordinates: { center, radius } };
...
function updateShapeData() {
  ...
  const f = selectedFeatureId ? features.find((x) => x.id === selectedFeatureId) || features[features.length - 1] : features[features.length - 1];
  if (!f) {
    el.value = '';
    updatePointCount(0);
    hideDelete();
  ...
  if (payload.shape_type === 'polygon' && payload.coordinates.points) updatePointCount(payload.coordinates.points.length);
...
/* ---------- Load/render existing geofences (same endpoints) ---------- */
...
function renderGeofencesOnMap() {
  ...
  geofences.forEach((gf) => {
    ...
    if (gf.shape_type === 'circle') {
      ...
      overlay = new google.maps.Circle({ center, radius: coords.radius, fillColor, fillOpacity, strokeColor, strokeWeight: 2, map: geofenceMap });
      const circleBounds = overlay.getBounds();
      if (circleBounds) bounds.union(circleBounds);
      hasGeofence = true;
    } else {
      const path = coords.points.map((p) => new google.maps.LatLng(p.lat, p.lng));
      overlay = new google.maps.Polygon({ paths: path, fillColor, fillOpacity, strokeColor, strokeWeight: 2, map: geofenceMap });
...
function renderGeofenceList() {
  ...
  geofences.forEach((gf) => {
    ...
    metaDiv.innerHTML = `<div>Location: ${gf.location || 'N/A'}</div><div>Type: ${gf.shape_type}</div><div>Created by: ${gf.created_by}</div><div>Created on:<br>${new Date(gf.created_at).toLocaleString()}</div>`;
    const actions = document.createElement('div'); actions.className = 'geofence-actions';
    const viewBtn = document.createElement('button'); viewBtn.textContent = 'View'; viewBtn.className = 'view-btn'; viewBtn.onclick = () => zoomToGeofence(gf);
    const delBtn = document.createElement('button'); delBtn.textContent = 'Delete'; delBtn.className = 'delete-btn'; delBtn.onclick = () => deleteGeofence(gf._id);
    actions.appendChild(viewBtn); actions.appendChild(delBtn);
    content.appendChild(nameSpan); content.appendChild(metaDiv);
