...
imeiSearch.addEventListener("focus", function() {
...
document.addEventListener("click", function(e) {
  if (!imeiSearch.contains(e.target) && !searchSuggestions.contains(e.target)) {
    hideSuggestions();
  }
});
...
function fetchSuggestions(searchTerm) {
  fetch(`/deviceInvy/search_suggestions?imei=${searchTerm}`)
    .then(response => response.json())
    .then(data => {
      if (data.length > 0) {
...
function showSuggestions(devices) {
  searchSuggestions.innerHTML = '';
  devices.forEach(device => {
    const suggestionItem = document.createElement('div');
    suggestionItem.className = 'search-suggestion-item';
    suggestionItem.innerHTML = `
      <span class="imei">${device.IMEI}</span>
      <span class="details">${device.DeviceMake} ${device.DeviceModel}</span>
    `;
    suggestionItem.addEventListener('click', () => {
      imeiSearch.value = device.IMEI;
      hideSuggestions();
      searchDevices(device.IMEI);
    });
...
function searchDevices(imei) {
  const searchValue = imei || document.getElementById("imeiSearch").value.trim();
  if (!searchValue) {
    clearSearch();
    return;
  }
