...
def show_vehicle_data(vehicle_number):
    try:
        ...
        if not vehicle_data:
            return render_template('vehicle.html', vehicle_data=None, recent_data=None)
        ...
        return render_template('vehicle.html', vehicle_data=processed_data, recent_data=recent_data)
    except Exception as e:
...
@app.route("/vehicle/<imei>/alerts", methods=["GET"])
...
@app.route("/get_vehicle_path", methods=["GET"])
...
if __name__ == '__main__':
    ...
    app.run(host='64.227.137.175', port=8014, debug=True)