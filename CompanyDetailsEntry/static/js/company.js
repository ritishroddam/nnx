...
async function fetchNextCompanyID() {
  try {
    const response = await fetch("/companyDetailsEntry/next_company_id");
...
function saveCustomer(customerId) {
  ...
  fetch(`/companyDetailsEntry/edit_customer/${customerId}`, {
    method: "POST",
    headers: {
...
function deleteCustomer(customerId) {
  if (confirm("Are you sure you want to delete this customer?")) {
    fetch(`/companyDetailsEntry/delete_customer/${customerId}`, {
      method: "DELETE",
    })
      .then((response) => response.json())
      .then((data) => {
