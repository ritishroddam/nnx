...
def manual_entry():
    ...
    if len(data['MobileNumber']) != 10 :
        ...
        return redirect(url_for('SimInvyEntry.page'))
...
@simEntry_bp.route('/upload_file', methods=['POST'])
def upload_file():
...
@simEntry_bp.route('/edit_sim/<sim_id>', methods=['POST'])
def edit_sim(sim_id):
    try:
...
@simEntry_bp.route('/delete_sim/<sim_id>', methods=['DELETE'])
